...
Purpose: These four components are the remaining UI sections. SettingsPanel is the largest (~171 lines of JSX), LocationPressure handles GPS/pressure (~55 lines), EggInputs has egg parameter controls (~92 lines), and ResultDisplay shows results with timer button (~52 lines). Together they account for ~370 lines of JSX.
...
Extract lines 456-608 from egg-calculator.jsx (the settings panel content, inside the `{showSettings && ...}` conditional). The component renders the panel content only -- the parent controls visibility via conditional rendering.
...
Extract lines 665-719 from egg-calculator.jsx (the location & pressure section).
...
Extract lines 743-835 from egg-calculator.jsx. This includes: egg count buttons, water volume slider, egg size buttons + slider, and start temperature buttons.
...
Extract lines 613-662 from egg-calculator.jsx (the result display section inside the main card). This includes: egg visualization SVG, cooking time display, ideal time comparison, temperature drop warning, timer countdown inline display, and start/stop timer button.
...
The `getEggVisualization()` function (lines 237-250) should be moved INTO this component as a local function or inline JSX. It uses `consistency` and `CONSISTENCY_OPTIONS` to render the SVG.

Imports `useTranslation`, `CONSISTENCY_OPTIONS` from `../constants`, `formatTime`, `formatCountdown`, `formatTemp` from `../formatters`.

IMPORTANT for temp drop display (line 634): The inline Fahrenheit conversion `tempUnit === 'F' ? Math.round(tempDrop * 9 / 5) : tempDrop` must be preserved exactly as-is. Do NOT replace it with formatTemp (it's not a full temperature, it's a delta).
...
Copy JSX exactly. Export as named export. Add JSDoc.
  ...
  <done>EggInputs.jsx and ResultDisplay.jsx created with exact JSX, proper props interfaces, getEggVisualization moved into ResultDisplay, and all necessary imports.</done>
</task>
