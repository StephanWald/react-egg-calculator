...
const EggCalculator = () => {
  ...
  const toFahrenheit = (c) => Math.round((c * 9/5 + 32) * 10) / 10;
  const toCelsius = (f) => Math.round((f - 32) * 5/9 * 10) / 10;
  const toOz = (liters) => Math.round(liters * 33.814 * 10) / 10;
  const toLiters = (oz) => Math.round(oz / 33.814 * 10) / 10;
  const displayTemp = (c) => tempUnit === 'F' ? `${toFahrenheit(c)}°F` : `${c}°C`;
  const displayTempValue = (c) => tempUnit === 'F' ? toFahrenheit(c) : c;
  const displayTempDelta = (deltaC) => tempUnit === 'F' ? `${Math.round(deltaC * 9/5 * 10) / 10}°F` : `${deltaC}°C`;
  const displayVolume = (l) => volumeUnit === 'oz' ? `${toOz(l)} oz` : `${l}L`;
    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-100 p-4 md:p-8">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-6 relative">
          {/* Language Picker & Unit Toggles */}
          <div className="absolute right-0 top-0 flex items-center gap-2">
            {/* Temperature Unit Toggle */}
            <button
              onClick={() => setTempUnit(tempUnit === 'C' ? 'F' : 'C')}
              className="px-2 py-1 bg-white rounded-lg shadow-md hover:bg-gray-50 transition-colors text-sm font-medium"
          ...
          <span className="flex items-center gap-2 text-gray-700">
            ...
            {!showSettings && (
              <span className="text-sm text-gray-500">
                ({t(stoveTypes.find(s => s.id === stoveType)?.nameKey)}, {displayTemp(ambientTemp)})
              </span>
            )}
          </span>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="p-3 bg-blue-50 rounded-lg">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-sm font-bold text-blue-700 w-16 text-right">{displayTemp(waterStartTemp)}</span>
                  </div>
                  <div className="flex justify-between text-xs text-blue-500 mt-1">
                    <span>{t('waterTempCold')} {displayTemp(2)}</span>
                    <span>{t('waterTempWarm')} {displayTemp(40)}</span>
                  </div>
                </div>
                ...
                <div className="p-3 bg-orange-50 rounded-lg">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-sm font-bold text-orange-700 w-16 text-right">{displayTemp(ambientTemp)}</span>
                  </div>
                  <div className="flex justify-between text-xs text-orange-500 mt-1">
                    <span>{t('ambientWinter')} {displayTemp(-10)}</span>
                    <span>{t('ambientSummer')} {displayTemp(35)}</span>
                  </div>
                </div>
              </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8">
            ...
            {tempDrop !== null && tempDrop > 2 && (
              <div className="mt-4 p-3 bg-blue-50 rounded-lg text-sm text-center">
                <div className="text-blue-800">
                  ⚠️ {t('tempDropWarning')} <span className="font-bold">{displayTempDelta(tempDrop)}</span> {t('tempDropUnit')}
                </div>
                <div className="text-blue-600 text-xs mt-1">
                  {t('effectiveTemp')}: ~{displayTemp(effectiveTemp)}
                </div>
              </div>
            )}
          </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                <div className="flex items-center justify-center h-8 bg-white rounded-lg border border-sky-200">
                  <span className="text-lg font-bold text-sky-700">{displayTemp(boilingPoint)}</span>
                </div>
              </div>
              <div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {consistencyOptions.map((option) => (
                ...
                >
                  ...
                  <div className="text-xs text-gray-500">{displayTemp(option.temp)}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-5">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                {t('waterVolume')}: <span className="font-bold text-amber-600">{displayVolume(waterVolume)}</span>
              </label>
              <input
                type="range"
                min="0.5"
          ...
          <div className="mb-5">
            <label className="block text-sm font-medium text-gray-700 mb-2">
              {t('startTemp')}: <span className="font-bold text-amber-600">{displayTemp(startTemp)}</span>
            </label>
            ...
            <div className="mt-3 p-4 bg-emerald-50 rounded-xl">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="bg-white p-3 rounded-lg">
                  <div className="text-emerald-600 text-xs">{t('heatingPhase')} ({displayTemp(waterStartTemp)} → {displayTemp(boilingPoint)})</div>
              ...
              {ambientTemp < 15 && (
                <div className="mt-2 text-xs text-emerald-700">
                  {t('includesHeatLoss')} {displayTemp(ambientTemp)} {t('ambient')}
                </div>
              )}
            </div>
          )}
